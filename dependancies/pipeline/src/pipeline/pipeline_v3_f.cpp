...
//     pipeline.addFlop();
    pipeline.add(PipelineLambda(val, i, p) {
        ...
        reg->MAR = reg->PC;
        PipelinePrintStage(i) << "clock: " << reg->clock << ": " << PipelinePrintModifiersPrintValue(reg->MAR);
        reg->MDR = &p->instruction_memory.at(reg->MAR);
        PipelinePrintStage(i) << "clock: " << reg->clock << ": " << PipelinePrintModifiersPrintValue(*reg->MDR);
        reg->MDRPlusOne = &p->instruction_memory.at(reg->MAR+1);
        PipelinePrintStage(i) << "clock: " << reg->clock << ": " << PipelinePrintModifiersPrintValue(*reg->MDRPlusOne);
        reg->CIR = reg->MDR;
