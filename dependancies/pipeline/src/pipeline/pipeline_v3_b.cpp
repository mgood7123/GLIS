...
//                         input->push_back(std::move(functions.prev_flip_flop->pull_output()));
            ...
            // this section handles the HALT signal
            {
                ...
                if (functions.index_of_this_stage == 0) {
                    ...
                    PipelinePrintStage(functions.index_of_this_stage)
                        << "waiting on input_is_empty";
                    cvC->wait(this_unique_lock, [&input_is_empty, &recieved_halt, &functions, input, haltC] {
                        ...
                        {
//                             if (functions.prev_stage_is_flip_flop) {
                                // if previous stage is a flip-flop, then check its input
//                                 PipelinePrintStage(functions.index_of_this_stage)
//                                     << PipelinePrintBool(!functions.prev_flip_flop->hasOutput);
//                                 input_is_empty = !functions.prev_flip_flop->hasOutput;
//                             }
//                             if (!input_is_empty) {
                                if (functions.this_stage_is_flip_flop) {
...
//                             }
                        ...
                        // return true if both of these conditions are true, in order to
                        // stop waiting, otherwise, return false to keep waiting
                        // NOTE: we need to keep processing input if we still have input
                        // NOTE: this can be simplified to input_is_empty || (recieved_halt && input_is_empty)
                        //       this translates to:
                        //
                        //         if input is empty
                        //           then return true
                        //         otherwise false
                        //
                        //         if we have recieved a recieved halt
                        //         and our input is empty
                        //           then return true
                        //         otherwise return false
                        // 
                        // NOTE: our input_is_empty check must come BEFORE our recieved_halt check
                        PipelinePrintStage(functions.index_of_this_stage)
                            << PipelinePrintBool(input_is_empty);
                        PipelinePrintStage(functions.index_of_this_stage)
                            << PipelinePrintBool(recieved_halt);
                        PipelinePrintStage(functions.index_of_this_stage)
                            << PipelinePrintBool(input_is_empty || (recieved_halt && input_is_empty));
                        return input_is_empty || (recieved_halt && input_is_empty);
                    });
                    PipelinePrintStage(functions.index_of_this_stage)
                        << "waited on input_is_empty";
