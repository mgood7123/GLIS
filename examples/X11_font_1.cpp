...
struct microatlas {
    ...
    struct font_init {
        ...
        font_init &operator=(const font_init &p2) {
            puts("font_init copy assignment");
            fflush(stdout);
            strcpy(this->font_path, p2.font_path);
            this->ft_initialized = p2.ft_initialized;
            this->ft = p2.ft;
        ...
        font_init &operator=(font_init &&p2) {
            puts("font_init move assignment");
            fflush(stdout);
            std::swap(this->font_path, p2.font_path);
            std::swap(this->ft_initialized, p2.ft_initialized);
            std::swap(this->ft, p2.ft);
    ...
    struct font_data {
        ...
        font_data &operator=(const font_data &p2) {
            puts("font_data copy assignment");
            fflush(stdout);
            this->sizes = p2.sizes;
        ...
        font_data &operator=(font_data &&p2) {
            puts("font_data move assignment");
            fflush(stdout);
            std::swap(this->sizes, p2.sizes);
            return *const_cast<font_data*>(this);
        }
    ...
    bool add_font_size(const char * id, int size) {
        ...
        return fontData->sizes.newObject(0, 0, x) != nullptr;
    }
    ...
    void render_text(const char *text, atlas * a, float x, float y, float sx, float sy) {
        ...
        glBindTexture(GL_TEXTURE_2D, a->tex);
        glUniform1i(uniform_tex, 0);
        glUniformMatrix4fv(glGetUniformLocation(a->font_source->program, "projection"), 1, GL_FALSE, glm::value_ptr(glm::ortho(0.0f, static_cast<GLfloat>(width), 0.0f, static_cast<GLfloat>(height))));
    ...
    void render_text(const char *text, const char * font_id, const int font_size, float x, float y, float sx, float sy) {
        ...
        render_text(text, a, x, y, sx, sy);
    }

    void display(const char * id) {
        ATLAS_TYPE * data = get_atlas(id);
        if (data == nullptr) return;
        font_init * fontInit = data->third.get<font_init*>();
        atlas * a12 = find_size(id, data->fourth.get<font_data*>(), 12);
        atlas * a24 = find_size(id, data->fourth.get<font_data*>(), 24);
        atlas * a48 = find_size(id, data->fourth.get<font_data*>(), 48);
